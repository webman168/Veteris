...
- (id)initWithBinary:(NSString *)thePath {
    ...
    if (self = [super init]) {
        ...
        NSString *trimmedReplacement = [[[[Preferences sharedInstance] crackerName] componentsSeparatedByCharactersInSet:charactersToRemove1] componentsJoinedByString:@""];
...
- (NSString *)stripArch:(cpu_subtype_t)keep_arch {
    ...
    DebugLog(@"sinf file yo %@", sinfPath);
...
- (BOOL)removeArchitecture:(struct fat_arch *)removeArch {
    ...
    DebugLog(@"searching for copyindex");

    for (int i = 0; i < CFSwapInt32(fh->nfat_arch); i++) {
        DebugLog(@"index %u, nfat_arch %u", i, CFSwapInt32(fh->nfat_arch));
        if (CFSwapInt32(arch->cpusubtype) == CFSwapInt32(removeArch->cpusubtype)) {

            DebugLog(@"found the upperArch we want to remove!");
            fgetpos(lipoOut, &upperArchpos);

            // check the index of the arch to remove
            if ((i + 1) == CFSwapInt32(fh->nfat_arch)) {
                // it's at the bottom
                DebugLog(@"at the bottom!! capitalist scums");
                strip_is_last = true;
            } else {
                DebugLog(@"hola");
            }
    ...
    if (!strip_is_last) {
        ...
        for (int i = 0; i < CFSwapInt32(fh->nfat_arch); i++) {
            ...
            if (i == (CFSwapInt32(fh->nfat_arch)) - 1) {
                DebugLog(@"found the lowerArch we want to copy!");
        ...
        DebugLog(@"upperArchpos %lld, lowerArchpos %lld", upperArchpos, lowerArchpos);
...
- (BOOL)crackBinaryToFile:(NSString *)finalPath error:(NSError *__autoreleasing *)error {
    ...
    switch (fh->magic) {
        ...
        case MH_MAGIC: {
            ...
            if (local_arch == CPU_SUBTYPE_ARM_V7S) {
        ...
        case FAT_CIGAM: {
            ...
            for (int i = 0; i < CFSwapInt32(fh->nfat_arch); i++) {
                DebugLog(@"arch arch subtype %u", arch->cputype);
                arch++;
            }
            ...
            // loop + crack
            for (int i = 0; i < CFSwapInt32(fh->nfat_arch); i++) {
                DebugLog(@"currently cracking arch %u", CFSwapInt32(arch->cpusubtype));

                switch ([Device compatibleWith:arch]) {
                    case COMPATIBLE: {
                        DebugLog(@"arch compatible with device!");
                    ...
                    case NOT_COMPATIBLE: {
                        DebugLog(@"arch not compatible with device!");
                    ...
                    case COMPATIBLE_SWAP: {
                        DebugLog(@"arch compatible with device, but swap");
                        NSString *stripPath;
                        ...
                        if (stripPath == NULL) {
                            ...
                            if (error != NULL)
                                *error = [NSError errorWithDomain:@"BinaryDumpError" code:-1 userInfo:@{NSLocalizedDescriptionKey : @"Error stripping/swapping binary!"}];
                        ...
                        if (![self dumpOrigFile:stripBinary withLocation:stripPath toFile:newbinary withArch:*arch]) {
                            ...
                            if (error != NULL)
                                *error = [NSError errorWithDomain:@"BinaryDumpError" code:-1 userInfo:@{NSLocalizedDescriptionKey : [NSString stringWithFormat:@"Cannot crack stripped %@ portion of binary.", [self readable_cpusubtype:CFSwapInt32(arch->cpusubtype)]]}];
                ...
                if ((archCount - [stripHeaders count]) == 1) {
                    DebugLog(@"only one architecture left!? strip");
                    if (compatibleArch != NULL) {
                        ...
                        if (!lipoSuccess) {
                            ...
                            if (error != NULL)
                                *error = [NSError errorWithDomain:@"BinaryDumpError" code:-1 userInfo:@{NSLocalizedDescriptionKey : @"Could not lipo binary"}];
...
- (BOOL)dumpOrigFile:(FILE *)origin withLocation:(NSString *)originPath toFile:(FILE *)target withArch:(struct fat_arch)arch {
    DebugLog(@"currently cracking 32bit portion");
...
- (BOOL)dump64bitOrigFile:(FILE *)origin withLocation:(NSString *)originPath toFile:(FILE *)target withTop:(uint32_t)top patchPIE:(BOOL)patchPIE {
    ...
    for (int lc_index = 0; lc_index < mach.ncmds; lc_index++) { // iterate over each load command
        ...
        // DebugLog(@"command: %u", CFSwapInt32(l_cmd.cmd));
        if (l_cmd.cmd == LC_CODE_SIGNATURE) { // code signature?
            fseek(target, -1 * sizeof(struct load_command), SEEK_CUR);
            fread(&ldid, sizeof(struct linkedit_data_command), 1, target);
            DebugLog(@"found code signature");
    ...
    if ((pid = fork()) == 0) {
        ...
        if ((err = task_for_pid(mach_task_self(), pid, &port) != KERN_SUCCESS)) {
            DebugLog(@"ERROR: Could not obtain mach port, did you sign with proper entitlements?");
        ...
        if ((mach.flags & MH_PIE) && (!patchPIE)) {
            ...
            while (err == KERN_SUCCESS) {
                ...
                DebugLog(@"64-bit Region Size: %llu %u", region_size, crypt.cryptsize);
        ...
        while (togo > 0) {
            // TODO hook this into SVProgressHUD, Notification?
            ...
            if ((err = mach_vm_read_overwrite(port, (mach_vm_address_t)__text_start + (pages_d * 0x1000), (vm_size_t)0x1000, (pointer_t)buf, &local_size)) != KERN_SUCCESS) {
                DebugLog(@"dum_error: %u", err);
                DebugLog(@"dumping binary: failed to dump a page (64)");
...
- (BOOL)dump32bitOrigFile:(FILE *)origin withLocation:(NSString *)originPath toFile:(FILE *)target withTop:(uint32_t)top patchPIE:(BOOL)patchPIE {
    ...
    uint64_t __text_start = 0;
    // uint64_t __text_size = 0;
    DebugLog(@"32bit dumping, offset %u", top);
    ...
    for (int lc_index = 0; lc_index < mach.ncmds; lc_index++) { // iterate over each load command
        ...
        if (l_cmd.cmd == LC_ENCRYPTION_INFO) { // encryption info?
            ...
            DebugLog(@"found encryption info");
    ...
    // we need to have found both of these
    if (!foundCrypt || !foundSignature || !foundStartText) {
        DebugLog(@"dumping binary: some load commands were not found");
        return FALSE;
    }
    ...
    if ((pid = fork()) == 0) {
        ...
        if ((mach.flags & MH_PIE) && (!patchPIE)) {
            ...
            while (err == KERN_SUCCESS) {
                ...
                DebugLog(@"32-bit Region Size: %llu %u", region_size, crypt.cryptsize);
            ...
            if (err != KERN_SUCCESS) {
                free(checksum);
                DebugLog(@"32-bit mach_vm_error: %u", err);
                printf("ASLR is enabled and we could not identify the decrypted memory region.\n");
                kill(pid, SIGKILL);
                return FALSE;
            }
