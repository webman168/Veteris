...
static NSString *kVeterisMaintainerSettingsKey = @"credits_maintainer";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if ([specifier.type isEqualToString:kIASKPSToggleSwitchSpecifier]) {
		...
		cell.userInteractionEnabled = [specifier.key isEqualToString:kVeterisMaintainerSettingsKey];
        cell.selectionStyle = [specifier.key isEqualToString:kVeterisMaintainerSettingsKey] ? UITableViewCellSelectionStyleNone : UITableViewCellSelectionStyleBlue;
	}
        ...
        if (floor(NSFoundationVersionNumber) < NSFoundationVersionNumber_iOS_6_0) {
            switch (specifier.textAlignment) {
                case UITextAlignmentLeft: textField.textAlignment = NSTextAlignmentLeft; break;
                case NSTextAlignmentCenter: textField.textAlignment = NSTextAlignmentCenter; break;
                case UITextAlignmentRight: textField.textAlignment = NSTextAlignmentRight; break;
                default: break;
            }
        } else {
		    textField.textAlignment = NSTextAlignmentFromCTTextAlignment(specifier.textAlignment);
        }
	...
	if (![specifier.type isEqualToString:kIASKPSMultiValueSpecifier] && ![specifier.type isEqualToString:kIASKPSTitleValueSpecifier] && ![specifier.type isEqualToString:kIASKPSTextFieldSpecifier]) {
        if (floor(NSFoundationVersionNumber) < NSFoundationVersionNumber_iOS_6_0) {
            switch (specifier.textAlignment) {
                case UITextAlignmentLeft: cell.textLabel.textAlignment = NSTextAlignmentLeft; break;
                case NSTextAlignmentCenter: cell.textLabel.textAlignment = NSTextAlignmentCenter; break;
                case UITextAlignmentRight: cell.textLabel.textAlignment = NSTextAlignmentRight; break;
                default: break;
            }
        } else {
    		cell.textLabel.textAlignment = NSTextAlignmentFromCTTextAlignment(specifier.textAlignment);
        }
	}
    if (floor(NSFoundationVersionNumber) < NSFoundationVersionNumber_iOS_6_0) {
        switch (specifier.textAlignment) {
            case UITextAlignmentLeft: cell.detailTextLabel.textAlignment = NSTextAlignmentLeft; break;
            case NSTextAlignmentCenter: cell.detailTextLabel.textAlignment = NSTextAlignmentCenter; break;
            case UITextAlignmentRight: cell.detailTextLabel.textAlignment = NSTextAlignmentRight; break;
            default: break;
        }
    } else {
        cell.detailTextLabel.textAlignment = NSTextAlignmentFromCTTextAlignment(specifier.textAlignment);
    }
...
#pragma clang diagnostic pop
        ...
        _currentChildViewController = targetViewController;
        [[self navigationController] pushViewController:targetViewController animated:YES];
    } else if ([[specifier type] isEqualToString:kIASKPSTitleValueSpecifier] && [specifier.key isEqualToString:kVeterisMaintainerSettingsKey]) {
        [tableView deselectRowAtIndexPath:indexPath animated:YES];
